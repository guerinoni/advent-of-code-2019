use crate::{intcode::OpCode, solver::*};
 ...
 * current phase setting (an integer from 0 to 4). Each phase setting is used exactly once, but the Elves can't remember which amplifier needs which phase setting.
 *
 ...
 * amplifier B to setting 1, C to 2, D to 4, and E to 0. Then, you could determine the output signal that gets sent from amplifier E to the thrusters with the following steps:
 *
 * Start the copy of the amplifier controller software that will run on amplifier A. At its first input instruction, provide it
 * the amplifier's phase setting, 3. At its second input instruction, provide it the input signal, 0. After some calculations,
 * it will use an output instruction to indicate the amplifier's output signal.
 ...
 * The final output signal from amplifier E would be sent to the thrusters. However, this phase setting sequence may not have
 * been the best one; another sequence might have sent a higher signal to the thrusters.
 *
...
impl Solver for Day7 {
    fn new(input_file: &'static str) -> Day7 {
        Day7 {
            filename: input_file,
        }
    }
...
impl Day7 {
    ...
    fn process(&self, commands: &[i64], sequence: &[i64]) -> i64 {
        let mut signal = 0;
        for s in sequence {
            let mut cpu = crate::intcode::new(commands.into(), true);
            cpu.set_input(*s);
            cpu.do_step();
 ...
 * amplifier A's input. This creates the feedback loop: the signal will be sent through the amplifiers many times.
 *
 ...
 * input and produce output many times before halting. Provide each amplifier its phase setting at its
 ...
 * first signal and the very last signal. To start the process, a 0 signal is sent to amplifier A's input exactly once.
 *
 * Eventually, the software on the amplifiers will halt after they have processed the final loop.
 * When this happens, the last output signal from amplifier E is sent to the thrusters. Your job is to
 * find the largest output signal that can be sent to the thrusters using the new phase settings and feedback loop arrangement.
 *
 * Here are some example programs:
 *
...
impl Day7 {
    fn part2(&self, data: Vec<i64>) -> i64 {
        (5..=9)
            .permutations(5)
            .map(|sequence| self.feedback_loop(&data, &sequence))
            .max()
            .unwrap()
    }

    fn feedback_loop(&self, commands: &[i64], sequence: &[i64]) -> i64 {
        let mut cpus = Vec::new();
        for (idx, &s) in sequence.iter().enumerate() {
            cpus.push(crate::intcode::new(commands.into(), true));
            cpus[idx].set_input(s);
            cpus[idx].do_step();
        }

        let mut signal = 0;
        for i in (0..5).cycle() {
            cpus[i].set_input(signal);
            cpus[i].run();
            if cpus[i].current_opcode() == OpCode::Halt {
                return signal;
            }
            signal = cpus[i].get_output();
        }
        signal
    }
}
...
mod tests {
    ...
    #[test]
    fn validate() {
        let d = Day7::new("input/day7");
        let data = file_with_comma_to_vec(d.filename);
        let r = d.part1(data.into());
        assert_eq!(r, 880726);
